...
def transfer_data(source: str, dest: str, num_row: int):
    ...
    """
    with open(source, 'r', encoding='utf-8') as source, open(dest, 'a', encoding='utf-8') as destination:
...
while True:
    ...
    if mode == 1:
        ...
        row = int(input("Введите номер переносимой строки: "))
        transfer_data(file, dest, row)